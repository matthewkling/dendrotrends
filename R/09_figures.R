...
compile_esr <- function(recr_pred, grow_pred, mort_pred){
      f <- bind_rows(recr_pred$esr %>% mutate(model = "recruitment",
            ...
            mutate(stat = factor(stat, levels = c("exposure", "sensitivity", "response")),
                   ...
                   value = ifelse(stat == "sensitivity", y2000, delta))
...
exposure_plots <- function(f, e, species){
      ...
      # ee <- read_csv("data/env_trends.csv") %>%
      ...
      ell <- ee %>%
            ...
            spread(year, value) %>%
            mutate(mean = y2020/2 + y2000/2,
                   delta = y2020 - y2000,
                   rate = delta / 20,
                   rate_prop = ifelse(var == "temperature", rate, (y2020 / y2000) ^ (1/20) - 1)) %>%
            group_by(var) %>%
...
sensitivity_plots <- function(f, species, recr_draws, grow_draws, mort_draws){
      ...
      oscl <- f %>%
            filter(stat == "response") %>%
            mutate(y2010 = y2000/2 + y2020/2) %>%
            group_by(model, param) %>%
            summarize(mean = mean(y2010, na.rm = T),
                      sd = sd(y2010, na.rm = T))
      oscl2 <- oscl %>%
...
response_plots <- function(f, e){
      ...
      pd <- f %>% ungroup() %>%
            ...
            summarize(y2020 = sum(y2020),
                      y2000 = sum(y2000), .groups = "drop")

      # inv_logit <- function(x) 1 / (1 + exp(-x))

      pd <- pd %>%
            mutate(y2000 = case_when(param == "mu" ~ exp(y2000),
                                     param == "zeta" ~ inv_logit(y2000),
                                     param == "beta" ~ y2000),
                   y2020 = case_when(param == "mu" ~ exp(y2020),
                                     param == "zeta" ~ inv_logit(y2020),
                                     param == "beta" ~ y2020)) %>%
            group_by(i, species, model, plot_id, tree_id) %>%
            summarize(y2000 = prod(y2000), # multiply mu and zeta to compute overall recruitment
                      y2020 = prod(y2020),
                      value = (y2020 - y2000) / 20,
                      n = n(),
                      .groups = "drop")
      ...
      response_plot_native <- function(m = "growth"){
            ...
            cdens <- pdll %>% filter(model == m) %>%
                  mutate(value = clamp(value, lims[1], lims[2])) %>%
                  ggplot(aes(value, 0, fill = after_stat(x), height = after_stat(density))) +
                  geom_density_ridges_gradient(adjust = .5, stat = "density", linewidth = .25) +
...
importance_plots <- function(f, species){
      ...
      am <- f %>%
            select(-plot_id, -tree_id, -t, -y2000, -y2020, -delta, -param) %>%
            filter(var != "combined") %>%
