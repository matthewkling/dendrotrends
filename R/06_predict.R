...
predict_growth <- function(grow_draws, grow_data, trends, ndraws){
      ...
      fm <- left_join(fe, mt, by = join_by(species, plot_id, year)) %>%
            ...
            spread(stat, value) %>%
            mutate(drdt_pred = (pred_2010 - pred_2009) / dt, # option A: delta sqrt-link variable
                   drdt_obs = (obs_2010 - obs_2009) / dt) %>%
            # mutate(drdt_pred = (inv_trans(pmax(0, pred_2010)) - inv_trans(pmax(0, pred_2009))) / 20, # option B: delta linear variable
            #        drdt_obs = (inv_trans(pmax(0, obs_2010)) - inv_trans(pmax(0, obs_2009))) / 20) %>%
            filter(nreps > 1)
