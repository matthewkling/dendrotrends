...
# what explains varition in accuracy?
      ...
      err_data <- function(x){
            x %>%
                  summarize(correlation = weightedCorr(drdt_pred, drdt_obs, "Pearson", weights = tree_yrs),
                            ...
                            accuracy_pct = -sqrt(weighted.mean((drdt_pred/weighted.mean(pred_2000/2+pred_2020/2, tree_yrs) -
                                                                      drdt_obs/weighted.mean(obs_2000/2+obs_2020/2, tree_yrs))^2, tree_yrs)),
                            slope = coef(lm(drdt_pred ~ drdt_obs))[2],
                            n_sqrt = sqrt(sum(tree_yrs)),
                            obs = weighted.mean(obs_2000/2 + obs_2020/2, tree_yrs),
